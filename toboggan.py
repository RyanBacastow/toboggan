...
class Toboggan:
    ...
    def __init__(self):
        ...
        self.data_types = [{"name": "NUMBER",
                            "desc": "Used to store whole numbers. Has default precision and scale of 38 and 0 respectively."},
                           {"name": "FLOAT",
                            "desc": "Snowflake employs double-precision IEEE 754 floating-point numbers. It also supports special values like NaN (Not a Number), inf (infinity) and -inf(negative infinity)."},
                           {"name": "VARCHAR",
                            "desc": "It holds Unicode characters and has a maximum length of 16 MB. Some BI/ETL tools may initialize the maximum length of the VARCHAR data in storage or in memory."},
                           {"name": "BINARY",
                            "desc": "The BINARY data type does not have the concept of Unicode characters, so its length is always measured in bytes. The maximum length is 8 MB."},
                           {"name": "BOOLEAN",
                            "desc": "has 2 values TRUE or FALSE. It may also have an “unknown” value, which is displayed by NULL."},
                           {"name": "DATE",
                            "desc": "Snowflake provides support for a DATE data type (with no time elements). It allows dates in the most common forms (YYYY-MM-DD, DD-MON-YYYY, etc.)."},
                           {"name": "TIME",
                            "desc": "Snowflake provides support for a TIME data type in the form of HH:MM:SS. It also supports an optional precision parameter for fractional seconds. By default, the precision is 9. All TIME values should lie between 00:00:00 and 23:59:59.999999999."},
                           {"name": "TIMESTAMP",
                            "desc": "TIMESTAMP is a user-specified alias for one of the TIMESTAMP_* variants. Every operation where TIMESTAMP is employed, the associated TIMESTAMP_* variation is used. This data type is never stored in tables."},
                           {"name": "VARIANT",
                            "desc": "It is a universal data type, which can be used to store values of any other type, including OBJECT and ARRAY. It can store data up to a maximum size of 16 MB."},
                           {"name": "OBJECT",
                            "desc": "It is used to store collections of key-value pairs, where the key will be a non-empty string and the value is of VARIANT type. Currently, Snowflake does not support explicitly-typed objects."},
                           {"name": "ARRAY",
                            "desc": "It is used to display dense and sparse arrays of arbitrary size. An index is a non-negative integer (up to 2^31-1) and values are of VARIANT type. Currently, Snowflake does not provide support for fixed-size arrays or arrays with values of a specific non-VARIANT type."},
                           {"name": "GEOGRAPHY",
                            "desc": "models Earth as if it were a perfect sphere. It follos the WGS 84 standard."}]
            ...
            if mode:
                try:
                    ...
                    if f:
                        ...
                        ans = self.yes_no(
                            f"No files in {self.in_out_path}/{self.json_out_dir} to import. Re enter paths?")
                        if ans:
                            continue
                        else:
                            break
                    ...
                    print(e)
                    ans = self.yes_no(
                        f"{self.in_out_path}/{self.json_out_dir} does not exist. No import will be possible from this location. Should it be created?")
                    if ans:
                        continue
                    else:
                        break
    ...
    def intake_warehouses(self):
        """
        intake warehouses for storage in json docs
        see https://docs.snowflake.com/en/sql-reference/sql/create-warehouse.html
        """
        ...
        while True:
            ...
            if ans:
                ...
                auto_resume = self.make_choices(['true', 'false'],
    ...
    def intake_roles(self):
        """
        intake roles for storage in json docs
        see https://docs.snowflake.com/en/sql-reference/sql/create-role.html
        """
    ...
    def intake_databases(self):
        """
        intake databases for storage in json docs
        see https://docs.snowflake.com/en/sql-reference/sql/create-database.html
        """
    ...
    def intake_schemas(self):
        """
        intake schemas for storage in json docs
        see https://docs.snowflake.com/en/sql-reference/sql/create-schema.html
        """
        ...
        while True:
            ...
            if ans:
                ...
                databases = []
                while True:
                    out_dict = {'name': name, 'comment': comment, 'database': None, 'namespace': '', 'usage_access': [],
                                'all_access': []}
                    ans = self.yes_no(f"Do you want to add {name} to a database?")
                    if ans:
                        database = self.make_choices(self.database_names,
                                                     f"Which database should {name} be created in?", mode='help')
                    ...
                    while True:
                        ...
                        if ans:
                            role = self.make_choices(self.role_names,
                                                     f"What role should have access to objects in {database}.{name}?",
                                                     mode='help')
                            ...
                            # ROLE-TYPE SELECTION
                            access_type = self.make_choices(['ALL', 'USAGE'],
                                                            f"What type of access would you like to grant to {role} on {database}.{name}?",
                                                            mode='help')
                            ...
                            if access_type == 'ALL':
                                ...
                                print(
                                    "USAGE: Granting USAGE access enables using a schema, including executing SHOW SCHEMAS commands to list the schema details in a database.")
                                continue
    ...
    def intake_users(self):
        """
        intake users for storage in json docs
        see https://docs.snowflake.com/en/sql-reference/sql/create-user.html
        """
    ...
    def add_namespaces(self, name):
        """
        add namespace method for table intake
        """
        namespaces = []
        while True:
    ...
    def add_columns(self, name):
        """
        add column while loop method for column intake in internal tables
        """
        columns = []
        while True:
            ...
            if ans:
                ...
                if column_name in [x['column_name'] for x in columns]:
                    ...
                    column_type = self.make_choices([x['name'] for x in self.data_types],
                                                    f"What type of column should {column_name} be?")
                    if column_type == 'HELP':
                        print(f"{self.set}DATA TYPES{self.sep}")
                        print(
                            "These represent the most basic data types (though there are others as you can access in the docs here: https://docs.snowflake.com/en/sql-reference/data-types.html)")
    ...
    def intake_tables(self):
        """
        intake internal table objects for storage in json docs
        ...
        while True:
            ...
            if ans:
                ...
                self.table_names = [x['name'] for x in tables]
                break

        return tables

    def intake_external_tables(self):
        """
        intake external table objects for storage in json docs
        see https://docs.snowflake.com/en/sql-reference/sql/create-external-table.html
        """
        print(f"\n{self.sep}EXTERNAL TABLES{self.sep}\n")
        print("Add external table objects.\n")
        tables = self.external_tables.copy()
        while True:
            ans = self.yes_no(f"Do you want to add an external table?")

            if ans:
                out_dict = {"name": "", "namespace": "", "file_format": "", "stage": "", "auto_refresh": "TRUE",
                            "comment": ""}
                name = self.clean_text(input("\nWhat is the external table name?\n"), mode='upper')
                if name in [x['name'] for x in tables]:
                    print("\nTable already added, please use unique names.\n")
                    continue
                elif name == '':
                    ...
                    namespace = self.make_choices(self.namespaces, f"What namespace does {name} belong to?")
                    out_dict['namespace'] = namespace

                    stage = self.make_choices(self.stage_names, "What is the stage name this will attach to?",
                                              mode="help")
                    out_dict['stage'] = stage

                    auto_refresh = self.make_choices(['TRUE', 'FALSE'],
                                                     f"Should auto-refresh be enabled for this external table?")
                    out_dict['auto_refresh'] = auto_refresh

                    file_format = self.make_choices(['PARQUET', 'JSON', 'AVRO', 'CSV', 'ORC', 'XML'],
                                                    f"What is the file type for {name}?")
                ...
                self.external_table_names = [x['name'] for x in tables]
                break
    ...
    def add_integration_locations(self, mode="allowed", cloud="s3"):
        """
        Add specifically allowed and blocked cloud integration paths
        ...
        while True:
            ans = self.yes_no(
                f"Would you like to add a blob location to the {mode} storage locations? Default is '*' for allowed and None for blocked.")
            if ans:
    ...
    def intake_storage_integrations(self):
        """
        intake storage integration objects for storage in json docs
        see https://docs.snowflake.com/en/sql-reference/sql/create-storage-integration.html
        """
        ...
        while True:
            ...
            if ans:
                ...
                if name in [x['name'] for x in storage_integrations]:
                    ...
                    if cloud == "AWS":
                        integration_str = input(
                            "What is the storage aws role arn from AWS? (ex: arn:aws:iam::{account_number}:role/{role_name})\n")
                ...
                self.storage_integration_names = [x['name'] for x in storage_integrations]
                return storage_integrations

    def intake_stages(self):
        """
        intake stage objects for storage in json docs
        see https://docs.snowflake.com/en/user-guide/data-load-s3-create-stage.html
        """
        ...
        while True:
            ...
            if ans:
                ...
                if name in [x['name'] for x in stages]:
                    ...
                    integration = self.make_choices(self.storage_integration_names,
                                                    "What is the storage integration name?", mode="help")
                    out_dict['integration'] = integration

                    url = input(
                        f"What is the url path to the directory (s3://bucket/dir/subdir; azure://bucket/dir/subdir)\n")
                    out_dict['url'] = url
                ...
                self.stage_names = [x['name'] for x in stages]
                return stages

    def intake_pipes(self):
        """
        intake pipe objects for storage in json docs
        see https://docs.snowflake.com/en/sql-reference/sql/create-pipe.html
        """
        ...
        while True:
            ...
            if ans:
                ...
                if name in [x['name'] for x in pipes]:
                    ...
                    namespace = self.make_choices(table['namespaces'],
                                                  f"What database and schema does {name} belong to?")

                    full_namespace = f"{namespace}.{table_name}"
                    out_dict['namespace'] = namespace
                    out_dict['full_namespace'] = full_namespace
                ...
                self.pipe_names = [x['name'] for x in pipes]
                return pipes

    def intake_streams(self):
        """
        intake stream objects for storage in json docs
        see https://docs.snowflake.com/en/sql-reference/sql/create-stream.html
        """
        print(f"\n{self.sep}STREAMS{self.sep}\n")
        print("Add stream objects.\n")
        ...
        while True:
            ...
            if ans:
                ...
                self.stream_names = [x['name'] for x in streams]
                return streams

    def intake_tasks(self):
        """
        intake task objects for storage in json docs
        ...
        while True:
            ...
            if ans:
                ...
                schedule = self.make_choices(["5 MINUTES", "15 MINUTES", "30 MINUTES", "60 MINUTES", "CRON"],
                                             f"What cadence should this task run on?")

                if schedule == 'CRON':
                    print("CRON not yet supported")
                    continue
                    # self.clean_text(input("\nPlease submit the cron syntax for the schedule\n"), mode='upper')

                wh = self.make_choices(self.warehouse_names, f"Which warehouse should this task use?")

                stream_name = self.make_choices(self.stream_names, f"Which stream should this task read from?")

                namespace = self.make_choices(self.namespaces, f"Which namespace should this task belong to?")

                table_name = self.make_choices(self.table_names, f"Which table should this task populate?")

                full_namespace = f"{namespace}.{table_name}"

                select_statement = input("\nWhat is the query logic to run?\n")

                comment = input(f"\nLeave a descriptive comment to describe {name}:\n")

                # select_statement = f"""SELECT {select_statement} FROM {namespace}.{stream_name} WHERE METADATA$ACTION = 'INSERT'"""
                ...
                self.task_names = [x['name'] for x in tasks]
                return tasks

    def choose_mode(self):
        """
        Allow users to choose which mode they want to run the program in.
        Expandable method for future enhancement/feature addition.
        """
        while True:
    ...
    def select_resource_name(self, resource, edit_type, l):
        """
        re-usable method for edit logic
        """
        while True:
            resource = self.make_choices(l, f"Which {resource.lower()} would you like to {edit_type.lower()}?",
                                         mode='help')
            if resource == 'HELP':
    ...
    def delete_resource(self, resource_name, in_list, key_field, change_type='delete', match=False, nested=False,
                        preserve=True):
        """
        intake delete edits with keyword sensitive behavior.
        resource_name
        in_list
        key_field
        change_type
    ...
    def intake_changes(self):
        """
        intake edits: adds/deletes/changes
        """
        ...
        while True:
            ...
            if ans:
                ...
                self.external_table_names = [x['name'] for x in self.external_tables]
                ...
                self.task_names = [x['name'] for x in self.tasks]
                self.stream_names = [x['name'] for x in self.streams]
                ...
                if choice == "HELP":
                    ...
                    if edit_type == 'HELP':
                        print(
                            "You can add a resource, delete a resource, or edit the naming of an existing resource. Will not drop actual resources, only from the creation scripts.")
                        continue
                    ...
                    if choice == 'WAREHOUSES':
                        if edit_type == 'ADD':
                            ...
                            self.users = self.delete_resource(resource_name, self.users, 'warehouse',
                        ...
                        if edit_type == 'ADD':
                            ...
                            self.users = self.delete_resource(resource_name, self.users, 'namespace',
                                                              change_type='null', match=True)
                            self.tables = self.delete_resource(resource_name, self.tables, 'namespaces', match=True,
                                                               nested=True, preserve=False)
                        ...
                        if edit_type == 'ADD':
                            ...
                            self.schemas = self.delete_resource(resource_name, self.schemas, 'usage_access',
                                                                nested=True)
                            ...
                            self.users = self.delete_resource(resource_name, self.users, 'default_role',
                                                              change_type='null')
                        ...
                        if edit_type == 'ADD':
                            ...
                            if not resource_name:
                                continue
                            self.storage_integrations = self.delete_resource(resource_name, self.storage_integrations,
                                                                             'name')
                    ...
                    elif choice == 'EXTERNAL TABLES':
                        if edit_type == 'ADD':
                            self.external_tables = self.intake_external_tables()
                        elif edit_type == 'DELETE':
                            resource_name = self.select_resource_name(choice, edit_type, self.external_table_names)
                            if not resource_name:
                                continue
                            self.external_tables = self.delete_resource(resource_name, self.external_tables, 'name')

                    elif choice == 'STREAMS':
                        if edit_type == 'ADD':
                            self.streams = self.intake_streams()
                        elif edit_type == 'DELETE':
                            resource_name = self.select_resource_name(choice, edit_type, self.stream_names)
                            if not resource_name:
                                continue
                            self.streams = self.delete_resource(resource_name, self.streams, 'name')

                    elif choice == 'TASKS':
                        if edit_type == 'ADD':
                            self.tasks = self.intake_tasks()
                        elif edit_type == 'DELETE':
                            resource_name = self.select_resource_name(choice, edit_type, self.task_names)
                            if not resource_name:
                                continue
                            self.tasks = self.delete_resource(resource_name, self.tasks, 'name')

                    else:
                        print(f"{choice} is not a feature yet. Please use toboggan to create another type of object.\n")
            else:
    ...
    def create_sql(self, filename, template, objects):
        """
        reusable sql string creation for sql out docs
        """
        out_str = ""
    ...
    def create_roles_sql(self):
        ...
        out_str += self.create_sql('roles', role_template_sql, [role for role in self.roles if
                                                                role['name'] not in [x['name'] for x in
                                                                                     self.default_roles]])
    ...
    def create_grants_sql(self, mode='schema'):
        ...
        if mode == 'schema':
            for role in self.role_names:
                for schema in self.schemas:
                    if role in schema['all_access']:
                        ...
                        sec_admin_stmts.append(
                            f"GRANT SELECT ON FUTURE TABLES IN SCHEMA {schema['namespace']} TO ROLE {role};")
            ...
            for user in self.users:
                ...
                out_str += f"GRANT USAGE, OPERATE ON WAREHOUSE {user['warehouse']} TO {user['name']};\n"
    ...
    def create_tables_sql(self):
        ...
        for table in self.tables:
            ...
            for column in table['columns']:
                table_blob += table_insert_template_sql.render(column_name=column['column_name'],
    ...
    def create_external_tables_sql(self):
        """
        Add tables to external_tables.sql
        """

        out_str = """USE ROLE SYSADMIN;\n"""

        for table in self.external_tables:
            out_str += external_table_template_sql.render(table)

        self.write_sql('external_tables', out_str)
    ...
    def create_streams_sql(self):
        """
        Create streams.sql
        """
        out_str = "USE ROLE SYSADMIN;\n"
        for stream in self.streams:
            out_str += stream_template_sql.render(stream)

        self.write_sql('streams', out_str)
        return out_str

    def create_tasks_sql(self):
        """
        Create tasks.sql
        """
        out_str = "USE ROLE SYSADMIN;\n"
        for task in self.tasks:
            out_str += task_template_sql.render(task)

        self.write_sql('tasks', out_str)
        return out_str

    def create_storage_integration_sql(self):
        """
        Create storage_integrations.sql
    ...
    def create_teardown_sql(self):
        ...
        for stage in self.stages:
            out_str += teardown_template_sql.render(object_type="STAGE", name=f'{stage["namespace"]}.{stage["name"]}')

        for pipe in self.pipes:
            out_str += teardown_template_sql.render(object_type="PIPE", name=f'{pipe["namespace"]}.{pipe["name"]}')

        for table in self.tables:
            for namespace in table['namespaces']:
                out_str += teardown_template_sql.render(object_type="TABLE",
                                                        name=f'{namespace}.{table["name"]}')

        for stream in self.streams:
            out_str += teardown_template_sql.render(object_type="TABLE",
                                                    name=f'{stream["namespace"]}.{stream["name"]}')

        for task in self.tasks:
            out_str += teardown_template_sql.render(object_type="TASK",
                                                    name=f"{task['namespace']}.{task['name']}")
    ...
    def intake(self):
        ...
        if self.mode == 'IMPORT':
            ...
            if import_mode == 'Import from sql connection to existing account.':
                ...
                self.external_tables = self.read_snowflake('external tables')
                self.tasks = self.read_snowflake('tasks')
                self.streams = self.read_snowflake('streams')
            else:
                ...
                self.external_tables = self.read_json('external_tables')
                self.tasks = self.read_json('tasks')
                self.streams = self.read_json('streams')
            ...
            print(f"{self.sep}Existing External Tables{self.sep}\n")
            for x in self.external_tables:
                print(json.dumps(x, indent=4))
            ...
            print(f"{self.sep}Existing Streams{self.sep}\n")
            for x in self.streams:
                print(json.dumps(x, indent=4))

            print(f"{self.sep}Existing Tasks{self.sep}\n")
            for x in self.tasks:
                print(json.dumps(x, indent=4))
        else:
        ...
        # de-duplicate lists
        self.warehouses = [i for n, i in enumerate(self.warehouses) if i not in self.warehouses[n + 1:]]
        self.roles = [i for n, i in enumerate(self.roles) if i not in self.roles[n + 1:]]
        self.databases = [i for n, i in enumerate(self.databases) if i not in self.databases[n + 1:]]
        self.schemas = [i for n, i in enumerate(self.schemas) if i not in self.schemas[n + 1:]]
        self.users = [i for n, i in enumerate(self.users) if i not in self.users[n + 1:]]
        self.tables = [i for n, i in enumerate(self.tables) if i not in self.tables[n + 1:]]
        self.storage_integrations = [i for n, i in enumerate(self.storage_integrations) if
                                     i not in self.storage_integrations[n + 1:]]
        self.stages = [i for n, i in enumerate(self.stages) if i not in self.stages[n + 1:]]
        self.pipes = [i for n, i in enumerate(self.pipes) if i not in self.pipes[n + 1:]]
        self.external_tables = [i for n, i in enumerate(self.external_tables) if i not in self.external_tables[n + 1:]]
        self.streams = [i for n, i in enumerate(self.streams) if i not in self.streams[n + 1:]]
        self.tasks = [i for n, i in enumerate(self.tasks) if i not in self.tasks[n + 1:]]
    ...
    def create(self):
        ...
        if ans:
            ...
            # External Tables
            self.external_tables_sql = self.create_external_tables_sql()
            self.write_json('external_tables', self.external_tables)

            # Streams
            self.streams_sql = self.create_streams_sql()
            self.write_json('streams', self.streams)

            # Tasks
            self.tasks_sql = self.create_tasks_sql()
            self.write_json('tasks', self.tasks)
